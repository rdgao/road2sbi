...
- All four full-tutorial notebooks are committed (`2befe9f`) with outputs, and run top to bottom (Ex 0–2 in <20 s each, Ex 3 ~2 min on CPU). They were generated by a build script that no longer exists, so **edit the notebooks directly**.
- **Next:** the user reviews all four in detail and requests edits. After they confirm the full versions, extract the SOLUTION blocks into student versions. The format is still undecided (hidden/collapsed cells vs. separate `.py` solution files vs. a `solutions/` folder); ask before building.
